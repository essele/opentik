...
/*
 * Remove the service from the list and undo the fdsets
 */
int remove_service(lua_State *L) {
	...
	// Remove any occurences...
	for(i=0; i < service_slots; i++) {
		fprintf(stderr, "   >> slot %d = %p\n", i, services[i]);
		if(services[i] == us) services[i] = NULL; 
	}

	// Clean up fdset...
	if(us->read_func) FD_CLR(us->fd, &m_read);
	if(us->write_func) FD_CLR(us->fd, &m_write);

	// Recalc max_fd
	max_fd = mosq_fd;
	for(i=0; i < service_slots; i++) if(services[i] && services[i]->fd > max_fd) max_fd = services[i]->fd;
